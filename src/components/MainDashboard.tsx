...
const MainDashboard: React.FC = () => {
  ...
  const getGreeting = () => {
    ...
    if (role === 'receptionist') {
      return 'Good afternoon, Receptionist!';
    } else if (role === 'admin' || role === 'hospital') {
      return 'Good afternoon, Healthcare Practitioners!';
    } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const t = await getTasks(businessId);
      setTasks(t);
      setLoadingTasks(false);

      setLoadingPatients(true);
      const p = await getPatients(businessId);
  ...
  const handleStatusChange = async (id: string, status: Task['status']) => {
    if (!businessId) return;
    try {
      await updateTask(id, { status }, businessId);
    ...
    <div className="min-h-screen p-6 space-y-10 bg-gray-50">
      ...
      <div className="px-6 py-8 bg-white shadow rounded-xl">
        ...
        <p className="text-sm text-gray-600">
          Welcome to your central hub for managing healthcare operations.
        </p>
        <div className="flex flex-wrap gap-4 mt-6">
          <button
            onClick={() => navigate('/doctor')}
            className="px-4 py-2 border border-gray-300 rounded-md text-brown-700 bg-gray-50 hover:bg-white"
          >
            Doctor's View
          </button>
          <button
      ...
      <div className="px-6 py-8 bg-white shadow rounded-xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={() => navigate('/slates')}
            className="text-sm underline text-brown-700 hover:text-yellow-600"
          >
            View All →
          </button>
        </div>
            ...
            {tasks.map(task => (
              ...
              >
                <div className="flex-1">
                  ...
                  {task.notes && <p className="text-sm italic text-gray-500">Note: {task.notes}</p>}
                </div>
                <div className="flex-shrink-0 ml-4">
                  <select
                    value={task.status}
                    onChange={e => handleStatusChange(task.id!, e.target.value as Task['status'])}
                    className={`px-4 py-2 rounded-full text-sm font-semibold border border-gray-300 cursor-pointer appearance-none min-w-[120px] text-center ${getStatusStyles(task.status)}`}
                    style={{
                      backgroundImage:
